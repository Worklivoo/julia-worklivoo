...
<body>
  ...
  const handleGerarQrCode = async () => {
    ...
    const externalReference = `FU_DINAMICO_${selectedPlan.id}_${settingsOwnerUserId}_${timestampUnix}`;
    ...
    while (Date.now() < limite && isMountedRef.current) {
      try {
        const { data } = await supabase
          ...
          .eq('user_id', owner)
          .maybeSingle();
        if ((data as any)?.followup_dinamico === true) {
          ativado = true;
          break;
        }
      } catch (err) {
        console.warn('[FU Dinâmico] Erro ao consultar ativação (tentando novamente):', err);
      }
